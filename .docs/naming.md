...
* **ветка** - `BB-XXX_task_name`
* **коммит/пулреквест** - `type: [BB-XXX] - краткое описание, отвечает на вопрос "что было сделано в коммите/пулреквесте?"`

**Номер задачи в коммите и в ветке пишем в верхнем регистре (не [BB-300], а [BB-300]), потому что так работает интеграция большинства таск менеджеров и гитхаба, которая позволит нам синхронизировать коммиты и не терять изменения, если будем подключать что-то**
...
Пример:  
`fix: [BB-123] - исправлена бага с добавлением товара`
`feat: [BB-456] - добавлен блок обновления категорий`
`chore: [BB-789] - выносит переиспользуемый компонент заголовка и его стили`
